  ...
  (b.prototype = {
    ...
    init: function(b) {
      ...
      (this.options = a.extend(!0, {}, this.defaults, b || {})),
        ...
        this.$elem.prepend(
          ["<", this.options.selectors.stationaryHeader, "/>"].join("")
        ),
        (this.$listWrapper = this.$elem.find(
          "." + this.options.classes.container
        )),
        (this.$fakeHeader = this.$elem
          .find(this.options.selectors.stationaryHeader)
          .eq(0)),
        this.$fakeHeader.addClass(this.options.classes.stationaryHeader),
        this.$elem
          .find(this.options.selectors.groupContainer)
          .each(function(a) {
    ...
    testPosition: function() {
      for (
        var b, c, d, e = this.$listWrapper.scrollTop(), f = 0;
        this.elems[f].listOffset - e <= 0 &&
        ((b = this.elems[f]),
        (d = b.listBottom - e),
        d < -b.headerHeight && (c = b),
        f++,
        !(f >= this.elems.length));

      );
      0 > d && d > -b.headerHeight
        ? (this.$fakeHeader.addClass(this.options.classes.hidden),
          a(b.list).addClass(this.options.classes.animated))
        : (this.$fakeHeader.removeClass(this.options.classes.hidden),
          b && a(b.list).removeClass(this.options.classes.animated)),
        b && this.$fakeHeader.text(b.headerText);
    }
  }),
