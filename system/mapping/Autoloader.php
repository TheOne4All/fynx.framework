<?php
	...
	**/
	...
	class Autoloader {
		...
		public function loadFiles($path, $step = 0, $ext = null, $dir_step = 0) {
			if(substr($path, -1) != '/'){ $path = $path.'/'; } //Add a '/' to the end of the path if not present
					...
					if(is_dir($path.$file)) {
						...
						if($file_ext == true){
							if(strpos ($file, '.js') == true) {
								...
								echo '<link rel="stylesheet" type="text/css" href="../'.$path.$file; if($file ==  '_print.css') { echo ' media="print"'; } echo '"/>'.PHP_EOL;
							}							
						} 
						if($ext == null){
